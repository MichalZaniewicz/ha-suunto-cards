...
`,Ce=[[/cycl|bik/i,"mdi:bike"],[/run/i,"mdi:run"],[/trek|hik/i,"mdi:hiking"],[/walk/i,"mdi:walk"],[/gym|strength|weight/i,"mdi:dumbbell"],[/swim/i,"mdi:swim"],[/ski/i,"mdi:ski"],[/row/i,"mdi:rowing"]];function Te(e){if(e)for(const[t,a]of Ce)if(t.test(e))return a;return"mdi:run-fast"}const Ne={"01":"mdi:weather-sunny","02":"mdi:weather-partly-cloudy","03":"mdi:weather-cloudy","04":"mdi:weather-cloudy","09":"mdi:weather-pouring",10:"mdi:weather-rainy",11:"mdi:weather-lightning",13:"mdi:weather-snowy",50:"mdi:weather-fog"};function Re(e){if(e>=60){const t=Math.floor(e/60),a=Math.round(e%60);return{value:`${t}:${String(a).padStart(2,"0")}`,unit:"h"}}return{value:String(Math.round(e)),unit:"min"}}function Me(e){const t=Math.round(60*e);return`${Math.floor(t/60)}:${String(t%60).padStart(2,"0")}`}function Pe(e,t){return new Intl.DateTimeFormat(t,{hour:"numeric",minute:"2-digit"}).format(e)}function Ve(e,t=0){const a=Number(e.toFixed(t));return 0===a?"±0":a>0?`+${a}`:String(a)}const je=[["year",31536e3],["month",2592e3],["day",86400],["hour",3600],["minute",60]];function He(e,t){const a=(e.getTime()-Date.now())/1e3,i=new Intl.RelativeTimeFormat(t,{numeric:"auto"});for(const[e,t]of je)if(Math.abs(a)>=t)return i.format(Math.round(a/t),e);return i.format(Math.round(a/60),"minute")}const De=new Set(["unknown","unavailable",""]);let Oe=class extends ze{static getConfigElement(){return document.createElement("suunto-device-editor")}static getStubConfig(){return{type:"custom:suunto-last-workout-card"}}setConfig(e){this._config=e,this._configuredDeviceId=e.device_id}getCardSize(){return 4}render(){if(!this._config||!this.hass)return G;this._syncTheme();const e=this._resolveEntities();if("error"in e)return e.error;const{map:t}=e,a=this.hass,i=e=>t[e]?a.states[t[e]]:void 0,s=i("last_activity");if(!s||De.has(s.state))return this._message("mdi:calendar-blank-outline",ke(a,"empty.last_workout.title"),ke(a,"empty.last_workout.subtitle"));const r=i("last_workout_start"),n=i("last_distance"),o=i("last_duration"),c=i("last_avg_hr"),l=i("last_max_hr"),d=i("last_avg_pace"),u=i("last_avg_speed"),h=i("last_pte"),p=i("last_epoc"),m=i("last_feeling"),v=i("last_tss"),g=i("last_cal_per_km"),_=i("last_workout_weather"),f=i("last_workout_tags"),y=i("last_workout_achievements"),b=o?Re(Number(o.state)):void 0,w=void 0===d&&void 0!==u,$=m&&!De.has(m.state)?Number(m.state):void 0,k=h&&!De.has(h.state)?Number(h.state):void 0,x=y?Number(y.state):0;return B`
      <ha-card @click=${()=>this._openMoreInfo(t.last_activity)}>
        ...
        <div class="stats">
          ${n?this._stat((Number(n.state)/1e3).toFixed(1),"km",ke(a,"stat.distance")):G}
          ${b?this._stat(b.value,b.unit,ke(a,"stat.duration")):G}
          ${d?this._stat(Me(Number(d.state)),"/km",ke(a,"stat.avg_pace")):w?this._stat(Number(u.state).toFixed(1),"km/h",ke(a,"stat.avg_speed")):G}
          ${c?this._stat(String(Math.round(Number(c.state))),"bpm",ke(a,"stat.avg_hr"),!0):G}
          ${l?this._stat(String(Math.round(Number(l.state))),"bpm",ke(a,"stat.max_hr"),!0):G}
          ${void 0!==k?B`
                <div class="stat">
                  <div class="stat-value">${k.toFixed(1)}</div>
                  <div class="stat-label">${ke(a,"stat.training_effect")}</div>
                  <div class="severity">
                    ${[1,2,3,4,5].map(e=>B`<i class=${e<=Math.round(k)?`on s${e}`:""}></i>`)}
                  </div>
                </div>
              `:G}
        </div>
        ...
        ${v||p||void 0!==$||g?B`
              <hr />
              <div class="secondary">
                ${v?this._secondary(String(Math.round(Number(v.state))),ke(a,"stat.tss")):G}
                ${p?this._secondary(Number(p.state).toFixed(1),ke(a,"stat.epoc")):G}
                ${void 0!==$?B`
                      <div class="sec-item">
                        <div class="feeling">
                          ${[1,2,3,4,5].map(e=>B`<i class=${e<=$?"on":""}></i>`)}
                        </div>
                        <div class="sec-label">${ke(a,"stat.feeling")}</div>
                      </div>
                    `:G}
                ${g?this._secondary(`${Math.round(Number(g.state))}`,ke(a,"stat.energy"),"kcal/km"):G}
              </div>
            `:G}
        ${_&&!De.has(_.state)?B`
              <div class="weather">
        ...
        ${f&&!De.has(f.state)||x>0?B`
              <div class="footer">
                ...
                ${x>0?B`
                      <span
                        class="chip accent"
                        title=${y?.attributes.route_ranking?ke(a,"achievement.rank",{rank:y.attributes.route_ranking}):""}
                      >
    ...
    `}_openMoreInfo(e){e&&ge(this,"hass-more-info",{entityId:e})}};function Le(e){return B`
    <div class="bar">
      ${e.map(e=>B`<div
            class="seg"
  ...
  `}function Fe(e,t,a=64,i=6){const s=Math.max(0,Math.min(100,e)),r=(a-i)/2,n=2*Math.PI*r,o=a/2;return B`
  ...
  `}Oe.styles=[Ae,Ee,n`
    ...
    `],e([pe()],Oe.prototype,"_config",void 0),Oe=e([le("suunto-last-workout-card")],Oe);const qe=e=>`var(--sc-zone-${e})`;let Be=class extends ze{static getConfigElement(){return document.createElement("suunto-device-editor")}static getStubConfig(){return{type:"custom:suunto-hr-zones-card"}}setConfig(e){this._config=e,this._configuredDeviceId=e.device_id}getCardSize(){return 3}render(){if(!this._config||!this.hass)return G;this._syncTheme();const e=this._resolveEntities();if("error"in e)return e.error;const{map:t}=e,a=this.hass,i=[];for(let e=1;e<=5;e++){const s=t[`last_zone${e}`],r=s?a.states[s]:void 0;r&&!Number.isNaN(Number(r.state))&&i.push({n:e,minutes:Number(r.state),lower:r.attributes.lower_limit_bpm,upper:r.attributes.upper_limit_bpm})}const s=i.reduce((e,t)=>e+t.minutes,0);if(0===i.length||s<=0)return this._message("mdi:heart-pulse",ke(a,"empty.hr_zones.title"),ke(a,"empty.hr_zones.subtitle"));const r=t.last_workout_start,n=r?a.states[r]:void 0,o=ke(a,"card.hr_zones.last_workout");return B`
      <ha-card class="static">
        <div class="header">
        ...
        ${Le(i.map(e=>({flexGrow:e.minutes,colorVar:qe(e.n),title:ke(a,"label.zone",{n:e.n})})))}
    ...
    `],e([pe()],Be.prototype,"_config",void 0),Be=e([le("suunto-hr-zones-card")],Be);const Ue=new Set(["unknown","unavailable",""]);let Ge=class extends ze{static getConfigElement(){return document.createElement("suunto-device-editor")}static getStubConfig(){return{type:"custom:suunto-sleep-readiness-card"}}setConfig(e){this._config=e,this._configuredDeviceId=e.device_id}getCardSize(){return 4}render(){if(!this._config||!this.hass)return G;this._syncTheme();const e=this._resolveEntities();if("error"in e)return e.error;const{map:t}=e,a=this.hass,i=e=>t[e]?a.states[t[e]]:void 0,s=i("sleep_duration");if(!s||Ue.has(s.state))return this._message("mdi:sleep",ke(a,"empty.sleep_readiness.title"),ke(a,"empty.sleep_readiness.subtitle"));const r=i("wake_time"),n=i("sleep_deep"),o=i("sleep_light"),c=i("sleep_rem"),l=i("sleep_quality"),d=i("sleep_spo2"),u=i("sleep_hrv"),h=i("hrv_baseline"),p=i("hrv_status"),m=i("resting_hr"),v=i("resting_hr_baseline"),g=i("readiness"),_=i("nap_duration"),f=g&&!Ue.has(g.state)?Number(g.state):void 0,y=void 0!==f?function(e,t){return t>=70?{colorVar:"var(--sc-good)",label:ke(e,"band.readiness.great")}:t>=40?{colorVar:"var(--sc-warn)",label:ke(e,"band.readiness.fair")}:{colorVar:"var(--sc-bad)",label:ke(e,"band.readiness.low")}}(a,f):void 0,b=u&&h&&!Ue.has(h.state)?Number(u.state)-Number(h.state):void 0,w=m&&v&&!Ue.has(v.state)?Number(m.state)-Number(v.state):void 0,$=[n&&!Ue.has(n.state)?{flexGrow:Number(n.state),colorVar:"var(--sc-sleep-deep)",title:ke(a,"label.deep")}:void 0,o&&!Ue.has(o.state)?{flexGrow:Number(o.state),colorVar:"var(--sc-sleep-light)",title:ke(a,"label.light")}:void 0,c&&!Ue.has(c.state)?{flexGrow:Number(c.state),colorVar:"var(--sc-sleep-rem)",title:ke(a,"label.rem")}:void 0].filter(e=>void 0!==e),k=Re(60*Number(s.state)),x=_&&!Ue.has(_.state)?Number(_.state):void 0,S=!!_?.attributes.date&&function(e){const t=new Date;return e.getFullYear()===t.getFullYear()&&e.getMonth()===t.getMonth()&&e.getDate()===t.getDate()}(new Date(_.attributes.date)),z={duration:`${k.value} ${k.unit}`};return B`
      <ha-card class="static">
        <div class="header">
        ...
        <div class="stats">
          ${l?this._stat(String(Math.round(Number(l.state))),"%",ke(a,"stat.quality")):G}
          ${u?this._stat(String(Math.round(Number(u.state))),"ms",void 0!==b?ke(a,"stat.hrv_delta",{delta:Ve(b)}):ke(a,"stat.hrv"),void 0!==b?b>=0?"good":"bad":void 0):G}
          ${m?this._stat(String(Math.round(Number(m.state))),"bpm",void 0!==w?ke(a,"stat.resting_hr_delta",{delta:Ve(w)}):ke(a,"stat.resting_hr"),void 0!==w?w<=0?"good":"bad":void 0):G}
          ${d?this._stat(String(Math.round(Number(d.state))),"%",ke(a,"stat.spo2")):G}
        </div>
        ...
        ${$.length?B`
              <div class="stages">
                ...
                <div class="stage-legend">
                  ${$.map(e=>{const t=Re(e.flexGrow);return B`
        ...
        ${p&&!Ue.has(p.state)||x?B`
              <div class="footer">
                ${p&&!Ue.has(p.state)?(()=>{const e=function(e,t){return"low"===t?{colorVar:"var(--sc-warn)",label:ke(e,"band.hrv.low")}:"high"===t?{colorVar:"var(--sc-pulse)",label:ke(e,"band.hrv.high")}:{colorVar:"var(--sc-good)",label:ke(e,"band.hrv.balanced")}}(a,p.state);return B`<span class="chip" style="color:${e.colorVar}"
                        ><ha-icon icon="mdi:heart-flash"></ha-icon>${e.label}</span
                      >`})():G}
                ${x?B`<span class="chip accent">
                      <ha-icon icon="mdi:power-sleep"></ha-icon>${ke(a,S?"chip.nap":"chip.nap_earlier",{minutes:x})}
                    </span>`:G}
              </div>
            `:G}
      </ha-card>
    ...
    `],e([pe()],Ge.prototype,"_config",void 0),Ge=e([le("suunto-sleep-readiness-card")],Ge);const Ie=new Set(["unknown","unavailable",""]);let We=class extends ze{static getConfigElement(){return document.createElement("suunto-device-editor")}static getStubConfig(){return{type:"custom:suunto-recovery-card"}}setConfig(e){this._config=e,this._configuredDeviceId=e.device_id}getCardSize(){return 3}render(){if(!this._config||!this.hass)return G;this._syncTheme();const e=this._resolveEntities();if("error"in e)return e.error;const{map:t}=e,a=this.hass,i=e=>t[e]?a.states[t[e]]:void 0,s=i("recovery_balance");if(!s||Ie.has(s.state))return this._message("mdi:battery-heart-variant",ke(a,"empty.recovery.title"));const r=i("is_recovering"),n=i("recovery_until"),o=i("recovery_time"),c=i("stress_state"),l=i("workout_today"),d=Number(s.state),u=function(e,t){return t>=60?{colorVar:"var(--sc-good)",label:ke(e,"band.recovery.well")}:t>=30?{colorVar:"var(--sc-warn)",label:ke(e,"band.recovery.partial")}:{colorVar:"var(--sc-bad)",label:ke(e,"band.recovery.low")}}(a,d),h="on"===r?.state;let p=ke(a,"band.recovery.fully");if(h&&n&&!Ie.has(n.state)){const e=new Date(n.state).getTime()-Date.now();if(e>0){const t=Re(e/6e4);p=ke(a,"band.recovery.recovering",{time:`${t.value} ${t.unit}`})}}return B`
      <ha-card class="static">
        <div class="header">
        ...
        ${c||o?B`
              <div class="stats two">
                ${c&&!Ie.has(c.state)?this._stat(c.state,"",ke(a,"stat.stress_level")):G}
                ${o&&!Ie.has(o.state)?this._stat(Number(o.state).toFixed(1),"h",ke(a,"stat.recovery_window")):G}
              </div>
            `:G}
        ${"on"===l?.state?B`<div class="footer"><span class="chip accent"><ha-icon icon="mdi:calendar-check"></ha-icon>${ke(a,"chip.workout_logged_today")}</span></div>`:G}
      </ha-card>
    ...
    `],e([pe()],We.prototype,"_config",void 0),We=e([le("suunto-recovery-card")],We);const Ze=new Set(["unknown","unavailable",""]);let Ke=class extends ze{constructor(){super(...arguments),this._history=[],this._historyFetchedAt=0}static getConfigElement(){return document.createElement("suunto-device-editor")}static getStubConfig(){return{type:"custom:suunto-training-load-card"}}setConfig(e){this._config=e,this._configuredDeviceId=e.device_id}getCardSize(){return 4}willUpdate(e){e.has("hass")&&this.hass&&this._config&&this._maybeFetchHistory()}async _maybeFetchHistory(){const e=this._resolveEntities();if("error"in e||!this.hass)return;const t=e.map.fitness_ctl;if(!t)return;const a=Date.now();if(!(t===this._historyEntityId&&a-this._historyFetchedAt<6e5)){this._historyEntityId=t,this._historyFetchedAt=a;try{const e=new Date(a-2592e6).toISOString(),i=await this.hass.callApi("GET",`history/period/${e}?filter_entity_id=${t}&no_attributes`),s=(i?.[0]??[]).map(e=>({t:new Date(e.last_updated??e.last_changed??"").getTime(),v:Number(e.state)})).filter(e=>Number.isFinite(e.t)&&Number.isFinite(e.v));this._history=s}catch{this._history=[]}}}render(){if(!this._config||!this.hass)return G;this._syncTheme();const e=this._resolveEntities();if("error"in e)return e.error;const{map:t}=e,a=this.hass,i=e=>t[e]?a.states[t[e]]:void 0,s=i("fitness_ctl");if(!s||Ze.has(s.state))return this._message("mdi:arm-flex",ke(a,"empty.training_load.title"),ke(a,"empty.training_load.subtitle"));const r=i("fatigue_atl"),n=i("form_tsb"),o=i("acwr"),c=n&&!Ze.has(n.state)?Number(n.state):void 0,l=void 0!==c?function(e,t){return t>5?{colorVar:"var(--sc-good)",label:ke(e,"band.form.fresh")}:t<-20?{colorVar:"var(--sc-bad)",label:ke(e,"band.form.very_fatigued")}:t<-5?{colorVar:"var(--sc-warn)",label:ke(e,"band.form.fatigued")}:{colorVar:"var(--sc-pulse)",label:ke(e,"band.form.neutral")}}(a,c):void 0,d=o&&!Ze.has(o.state)?Number(o.state):void 0,u=void 0!==d?function(e,t){return t>1.3?{colorVar:"var(--sc-bad)",label:ke(e,"band.acwr.high")}:t<.8?{colorVar:"var(--sc-warn)",label:ke(e,"band.acwr.low")}:{colorVar:"var(--sc-good)",label:ke(e,"band.acwr.safe")}}(a,d):void 0;return B`
      <ha-card class="static">
        <div class="header">
  ...
  `}(this._history,"var(--sc-amber)")}
        ...
        <div class="stats three">
          ...
          ${r?this._stat(Number(r.state).toFixed(0),ke(a,"stat.atl")):G}
          ${void 0!==c?this._stat(Ve(c,1),ke(a,"stat.tsb"),l?.colorVar):G}
        </div>
    ...
    `],e([pe()],Ke.prototype,"_config",void 0),e([pe()],Ke.prototype,"_history",void 0),Ke=e([le("suunto-training-load-card")],Ke);const Qe=new Set(["unknown","unavailable",""]),Je=["var(--sc-amber)","var(--sc-pulse)","var(--sc-good)","var(--sc-sleep-rem)","var(--sc-zone-4)","var(--sc-sleep-deep)"];let Ye=class extends ze{static getConfigElement(){return document.createElement("suunto-device-editor")}static getStubConfig(){return{type:"custom:suunto-week-stats-card"}}setConfig(e){this._config=e,this._configuredDeviceId=e.device_id}getCardSize(){return 4}render(){if(!this._config||!this.hass)return G;this._syncTheme();const e=this._resolveEntities();if("error"in e)return e.error;const{map:t}=e,a=this.hass,i=e=>t[e]?a.states[t[e]]:void 0,s=i("weekly_distance"),r=i("weekly_time"),n=i("workouts_7d"),o=i("workouts_30d"),c=i("lifetime_by_activity");if(!s&&!c)return this._message("mdi:calendar-week",ke(a,"empty.week_stats.title"));const l=(c?.attributes.activities??[]).slice().sort((e,t)=>t.distance_km-e.distance_km),d=l.slice(0,5),u=l.length-d.length;return B`
      <ha-card class="static">
        <div class="header">
        ...
        ${s||r||n?B`
              <div class="stats three">
                ${s&&!Qe.has(s.state)?this._stat(Number(s.state).toFixed(1),"km",ke(a,"stat.distance")):G}
                ${r&&!Qe.has(r.state)?this._stat(Number(r.state).toFixed(1),"h",ke(a,"stat.time")):G}
                ${n&&!Qe.has(n.state)?this._stat(n.state,"",ke(a,"stat.workouts")):G}
              </div>
            `:G}
        ...
        ${d.length?B`
              ...
              <div class="lifetime">
                ...
                ${Le(d.map((e,t)=>({flexGrow:e.distance_km,colorVar:Je[t%Je.length],title:e.activity})))}
                <div class="rows">
            ...
            `:G}
        ${o&&!Qe.has(o.state)?B`<div class="footer"><span class="chip">${ke(a,"chip.workouts_30d",{count:o.state})}</span></div>`:G}
      </ha-card>
    `}_stat(e,t,a){return B`
      <div class="stat">
        <div class="stat-value">${e}${t?B`<span class="unit">${t}</span>`:G}</div>
    ...
    `],e([pe()],Ye.prototype,"_config",void 0),Ye=e([le("suunto-week-stats-card")],Ye);const Xe=new Set(["unknown","unavailable",""]);let et=class extends ze{static getConfigElement(){return document.createElement("suunto-device-editor")}static getStubConfig(){return{type:"custom:suunto-today-card"}}setConfig(e){this._config=e,this._configuredDeviceId=e.device_id}getCardSize(){return 2}render(){if(!this._config||!this.hass)return G;this._syncTheme();const e=this._resolveEntities();if("error"in e)return e.error;const{map:t}=e,a=this.hass,i=e=>t[e]?a.states[t[e]]:void 0,s=i("daily_steps"),r=i("daily_energy"),n=i("current_hr"),o=i("workout_today"),c=i("is_recovering");if(!s&&!r&&!n)return this._message("mdi:pulse",ke(a,"empty.today.title"));const l=n&&!Xe.has(n.state)?Math.round(Number(n.state)):void 0;return B`
      <ha-card class="static">
        ...
        <div class="stats three">
          ${s&&!Xe.has(s.state)?this._stat(Number(s.state).toLocaleString(a.language),"",ke(a,"stat.steps")):G}
          ${r&&!Xe.has(r.state)?this._stat(Math.round(Number(r.state)).toLocaleString(a.language),"kcal",ke(a,"stat.energy")):G}
          ${void 0!==l?B`
                <div class="stat hr">
        ...
        ${"on"===o?.state||"on"===c?.state?B`
              <div class="footer">
                ${"on"===o?.state?B`<span class="chip accent"><ha-icon icon="mdi:calendar-check"></ha-icon>${ke(a,"chip.workout_today")}</span>`:G}
                ${"on"===c?.state?B`<span class="chip"><ha-icon icon="mdi:bed-clock"></ha-icon>${ke(a,"chip.recovering")}</span>`:G}
              </div>
            `:G}
      </ha-card>
    `}_stat(e,t,a){return B`
    ...
    `],e([pe()],et.prototype,"_config",void 0),et=e([le("suunto-today-card")],et);const tt=new Set(["unknown","unavailable",""]);let at=class extends ze{static getConfigElement(){return document.createElement("suunto-device-editor")}static getStubConfig(){return{type:"custom:suunto-lifetime-card"}}setConfig(e){this._config=e,this._configuredDeviceId=e.device_id}getCardSize(){return 3}render(){if(!this._config||!this.hass)return G;this._syncTheme();const e=this._resolveEntities();if("error"in e)return e.error;const{map:t}=e,a=this.hass,i=e=>t[e]?a.states[t[e]]:void 0,s=i("lifetime_distance"),r=i("lifetime_time"),n=i("lifetime_energy"),o=i("lifetime_workouts"),c=i("lifetime_days");return!s||tt.has(s.state)?this._message("mdi:trophy-variant",ke(a,"empty.lifetime.title")):B`
      <ha-card class="static">
        <div class="header">
          <div class="icon-badge"><ha-icon icon="mdi:trophy-variant"></ha-icon></div>
          <div class="title-block">
            <div class="title">${ke(a,"card.lifetime.title")}</div>
            <div class="subtitle">${ke(a,"card.lifetime.subtitle")}</div>
          </div>
        </div>

        <div class="stats three">
          ${this._stat(Number(s.state).toFixed(0),"km",ke(a,"stat.distance"))}
          ${r?this._stat(Number(r.state).toFixed(0),"h",ke(a,"stat.time")):G}
          ${n?this._stat(Math.round(Number(n.state)).toLocaleString(a.language),"kcal",ke(a,"stat.energy")):G}
          ${o?this._stat(o.state,"",ke(a,"stat.workouts")):G}
          ${c?this._stat(c.state,"",ke(a,"stat.active_days")):G}
        </div>
      </ha-card>
    `}_stat(e,t,a){return B`
      <div class="stat">
    ...
    `],e([pe()],at.prototype,"_config",void 0),at=e([le("suunto-lifetime-card")],at);let it=class extends ze{static getConfigElement(){return document.createElement("suunto-device-editor")}static getStubConfig(){return{type:"custom:suunto-recent-workouts-card"}}setConfig(e){this._config=e,this._configuredDeviceId=e.device_id}getCardSize(){return 5}render(){if(!this._config||!this.hass)return G;this._syncTheme();const e=this._resolveEntities();if("error"in e)return e.error;const{map:t}=e,a=this.hass,i=t.workouts_recent,s=i?a.states[i]:void 0,r=s?.attributes.workouts??[];return s&&0!==r.length?B`
      <ha-card class="static">
        <div class="header">
          ...
          <div class="title-block">
            <div class="title">${ke(a,"card.recent_workouts.title")}</div>
          </div>
        </div>
    ...
    `:this._message("mdi:format-list-bulleted",ke(a,"empty.recent_workouts.title"))}};it.styles=[Ae,Ee,n`
      .workout-row {
        display: grid;
    ...
    `],e([pe()],it.prototype,"_config",void 0),it=e([le("suunto-recent-workouts-card")],it);const st=new Set(["unknown","unavailable",""]);let rt=class extends ze{static getConfigElement(){return document.createElement("suunto-device-editor")}static getStubConfig(){return{type:"custom:suunto-elevation-card"}}setConfig(e){this._config=e,this._configuredDeviceId=e.device_id}getCardSize(){return 4}render(){if(!this._config||!this.hass)return G;this._syncTheme();const e=this._resolveEntities();if("error"in e)return e.error;const{map:t}=e,a=this.hass,i=e=>t[e]?a.states[t[e]]:void 0,s=i("last_ascent"),r=i("last_descent");if((!s||st.has(s.state))&&(!r||st.has(r.state)))return this._message("mdi:image-filter-hdr",ke(a,"empty.elevation.title"),ke(a,"empty.elevation.subtitle"));const n=i("last_ascent_time"),o=i("last_descent_time"),c=i("last_min_altitude"),l=i("last_max_altitude"),d=i("last_ascent_rate"),u=i("last_workout_start");return B`
      <ha-card class="static">
        <div class="header">
          <div class="icon-badge"><ha-icon icon="mdi:image-filter-hdr"></ha-icon></div>
          <div class="title-block">
            <div class="title">${ke(a,"card.elevation.title")}</div>
            <div class="subtitle">
              ${u?`${ke(a,"card.hr_zones.last_workout")} · ${He(new Date(u.state),a.language)}`:ke(a,"card.hr_zones.last_workout")}
            </div>
          </div>
        </div>

        <div class="stats three">
          ${s&&!st.has(s.state)?this._stat(Math.round(Number(s.state)).toString(),"m",ke(a,"stat.ascent")):G}
          ${r&&!st.has(r.state)?this._stat(Math.round(Number(r.state)).toString(),"m",ke(a,"stat.descent")):G}
          ${n&&!st.has(n.state)?(()=>{const e=Re(Number(n.state));return this._stat(e.value,e.unit,ke(a,"stat.ascent_time"))})():G}
          ${o&&!st.has(o.state)?(()=>{const e=Re(Number(o.state));return this._stat(e.value,e.unit,ke(a,"stat.descent_time"))})():G}
          ${c&&!st.has(c.state)?this._stat(Math.round(Number(c.state)).toString(),"m",ke(a,"stat.min_altitude")):G}
          ${l&&!st.has(l.state)?this._stat(Math.round(Number(l.state)).toString(),"m",ke(a,"stat.max_altitude")):G}
        </div>
        ...
        ${d&&!st.has(d.state)?B`
              <div class="footer">
                <span class="chip">
                  ...
                  ${ke(a,"stat.ascent_rate")}: ${Math.round(Number(d.state))} m/h
                </span>
              </div>
            `:G}
      </ha-card>
    ...
    `],e([pe()],rt.prototype,"_config",void 0),rt=e([le("suunto-elevation-card")],rt);const nt=new Set(["unknown","unavailable",""]);let ot=class extends ze{constructor(){super(...arguments),this._mapLoading=!1}static getConfigElement(){return document.createElement("suunto-device-editor")}static getStubConfig(){return{type:"custom:suunto-location-card"}}setConfig(e){this._config=e,this._configuredDeviceId=e.device_id}getCardSize(){return 5}render(){if(!this._config||!this.hass)return G;this._syncTheme();const e=this._resolveEntities();if("error"in e)return e.error;const{map:t}=e,a=this.hass,i=e=>t[e]?a.states[t[e]]:void 0,s=i("last_workout_location"),r=s?.attributes.latitude,n=s?.attributes.longitude,o=t.last_workout_location;if(!s||nt.has(s.state)||void 0===r||void 0===n)return this._message("mdi:map-marker",ke(a,"empty.location.title"),ke(a,"empty.location.subtitle"));const c=i("last_activity"),l=i("last_workout_start"),d=`https://www.google.com/maps?q=${r},${n}`,u=Te(c?.state);return o&&this._ensureMapElement(o,u),this._mapEl&&(this._mapEl.hass=a),B`
      <ha-card class="static">
        <div class="header">
          <div class="icon-badge"><ha-icon icon="mdi:map-marker"></ha-icon></div>
          <div class="title-block">
            <div class="title">${ke(a,"card.location.title")}</div>
            <div class="subtitle">
              ${c?B`${c.state}`:G}
              ${c&&l?B`<span class="sep">·</span>`:G}
              ${l?He(new Date(l.state),a.language):G}
            </div>
          </div>
        </div>
        ...
        <div class="footer-row">
          ...
          <a class="chip accent link" href=${d} target="_blank" rel="noopener noreferrer">
            ...
            ${ke(a,"location.open_in_maps")}
          </a>
        </div>
      </ha-card>
    `}async _ensureMapElement(e,t){const a=`${e}:${t}`;if(this._mapEl&&this._mapKey===a||this._mapLoading)return;const i={type:"map",auto_fit:!0,default_zoom:14,entities:[{entity:e,icon:t}]},s=window.loadCardHelpers;if(!s){if(!customElements.get("hui-map-card"))return;try{const e=document.createElement("hui-map-card");e.setConfig(i),this._mapKey=a,this._mapEl=e}catch{}return}this._mapLoading=!0;try{const e=(await s()).createCardElement(i);this._mapKey=a,this._mapEl=e}catch{}finally{this._mapLoading=!1}}};ot.styles=[Ae,Ee,n`
      .subtitle .sep {
        opacity: 0.45;
        margin: 0 3px;
      }
    ...
    `],e([pe()],ot.prototype,"_config",void 0),e([pe()],ot.prototype,"_mapEl",void 0),ot=e([le("suunto-location-card")],ot);const ct=new Set(["unknown","unavailable",""]);let lt=class extends ze{static getConfigElement(){return document.createElement("suunto-device-editor")}static getStubConfig(){return{type:"custom:suunto-fitness-card"}}setConfig(e){this._config=e,this._configuredDeviceId=e.device_id}getCardSize(){return 3}render(){if(!this._config||!this.hass)return G;this._syncTheme();const e=this._resolveEntities();if("error"in e)return e.error;const{map:t}=e,a=this.hass,i=e=>t[e]?a.states[t[e]]:void 0,s=i("vo2max");if(!s||ct.has(s.state))return this._message("mdi:lungs",ke(a,"empty.fitness.title"),ke(a,"empty.fitness.subtitle"));const r=i("estimated_vo2max"),n=i("fitness_age"),o=s.attributes.measured_at,c=s.attributes.measured_from;return B`
      <ha-card class="static">
        <div class="header">
          <div class="icon-badge"><ha-icon icon="mdi:lungs"></ha-icon></div>
          <div class="title-block">
            <div class="title">${ke(a,"card.fitness.title")}</div>
            <div class="subtitle">
              ${o?ke(a,"fitness.measured",{time:He(new Date(o),a.language),activity:c??""}):G}
            </div>
          </div>
        </div>

        <div class="stats three">
          ${this._stat(Number(s.state).toFixed(1),"ml/kg/min",ke(a,"stat.vo2max"))}
          ${r&&!ct.has(r.state)?this._stat(Number(r.state).toFixed(1),"ml/kg/min",ke(a,"stat.estimated_vo2max")):G}
          ${n&&!ct.has(n.state)?this._stat(String(Math.round(Number(n.state))),"",ke(a,"stat.fitness_age")):G}
        </div>
      </ha-card>
    `}_stat(e,t,a){return B`
      <div class="stat">
    ...
    `}_openMoreInfo(e){e&&ge(this,"hass-more-info",{entityId:e})}};ut.styles=[Ae,Ee,n`
      ha-card {
        gap: 8px;
      }
      .activity {
