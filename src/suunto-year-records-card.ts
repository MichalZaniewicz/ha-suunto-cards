...
/**
 * This year's personal bests - see suunto-month-records-card's own comment
 * for the full reasoning; identical shape, scoped to training_records_year.
 */
@customElement("suunto-year-records-card")
export class SuuntoYearRecordsCard extends SuuntoBaseCard {
  ...
  public static getStubConfig(): SuuntoCardConfig {
    return { type: "custom:suunto-year-records-card" };
  }
  ...
  protected render(): TemplateResult | typeof nothing {
    ...
    const rows = computePrRows(hass, map, "training_records_year").filter((r) => r.entry);

    if (!rows.length) {
      return this._message(
        "mdi:trophy-award",
        t(hass, "empty.year_records.title"),
        t(hass, "empty.year_records.subtitle")
      );
    }
...
declare global {
  interface HTMLElementTagNameMap {
    "suunto-year-records-card": SuuntoYearRecordsCard;
  }
}
