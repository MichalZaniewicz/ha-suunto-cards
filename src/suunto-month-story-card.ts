...
/**
 * A "month in review" narrative card - same layout as the lifetime
 * suunto-story-card, reading month_* sensors instead of lifetime_* ones.
 * A deliberately separate card rather than one card with a period toggle
 * (user's own call, 2026-09) - keeps each card's config trivial and lets
 * someone place only the periods they actually want on their dashboard.
 */
@customElement("suunto-month-story-card")
export class SuuntoMonthStoryCard extends SuuntoBaseCard {
  ...
  public static getStubConfig(): SuuntoCardConfig {
    return { type: "custom:suunto-month-story-card" };
  }
  ...
  protected render(): TemplateResult | typeof nothing {
    ...
    const distance = get("month_distance");
    if (!distance || UNAVAILABLE_STATES.has(distance.state)) {
      return this._message("mdi:calendar-month", t(hass, "empty.month_story.title"));
    }

    const time = get("month_time");
    const energy = get("month_energy");
    const workouts = get("month_workouts");
    const days = get("month_active_days");
    ...
    const records = get("training_records_month");
    const streakDays =
      ...
      <ha-card class="static">
        <div class="header">
          ...
          <div class="title-block">
            <div class="title">${t(hass, "card.month_story.title")}</div>
            <div class="subtitle">${new Date().toLocaleDateString(hass.language, { month: "long", year: "numeric" })}</div>
          </div>
        </div>
                ...
                <div>
                  ...
                  <div class="t2">
                    ${mainActivityWorkouts !== undefined && mainActivityPct !== undefined
                      ? t(hass, "story.share_month", { count: mainActivityWorkouts, pct: mainActivityPct })
                      : nothing}
                  </div>
                </div>
              </div>
          ...
          ? html`
              <div class="story-block">
                ...
                <div>
                  ...
                  <div class="t2">${t(hass, "story.record_subtitle_month")}</div>
                </div>
              </div>
            `
          : nothing}
...
declare global {
  interface HTMLElementTagNameMap {
    "suunto-month-story-card": SuuntoMonthStoryCard;
  }
}
